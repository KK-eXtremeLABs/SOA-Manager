...
export class AssociationService {
  ...
  getList(filter: any = {}, page = 0, limit = 0): Observable<any> {
    ...
    return this.http.get(environment.adminUrl + 'associations', { params });
  }

  getOne(id: number, include: any = null): Observable<any> {
    const params: any = {};
    if (include) params.include = JSON.stringify(include);
    return this.http.get(environment.adminUrl + `associations/${id}`, { params });
  }

  create(data: any): Observable<any> {
    return this.http.post(environment.adminUrl + 'associations', data);
  }

  update(id: any, data: any): Observable<any> {
    return this.http.put(environment.adminUrl + `associations/${id}`, data);
  }

  delete(id: number): Observable<any> {
    return this.http.delete(environment.adminUrl + `associations/${id}`);
  }
}
