...
export class PricingService {
  ...
  getList(filter: any = {}, page = 0, limit = 0): Observable<any> {
    ...
    return this.http.get(environment.adminUrl + 'pricing', { params });
  }

  getOne(id: number, include: any = null): Observable<any> {
    const params: any = {};
    if (include) params.include = JSON.stringify(include);
    return this.http.get(environment.adminUrl + `pricing/${id}`, { params });
  }

  create(data: any): Observable<any> { return this.http.post(environment.adminUrl + 'pricing', data); }
  update(id: any, data: any): Observable<any> { return this.http.put(environment.adminUrl + `pricing/${id}`, data); }
  delete(id: any): Observable<any> { return this.http.delete(environment.adminUrl + `pricing/${id}`); }
}
