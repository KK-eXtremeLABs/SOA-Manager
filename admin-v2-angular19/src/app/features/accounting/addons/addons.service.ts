...
export class AddonsService {
  ...
  getList(filter: any = {}, page = 0, limit = 0): Observable<any> {
    ...
    return this.http.get(environment.adminUrl + 'addons', { params });
  }

  getOne(id: number, include: any = null): Observable<any> {
    const params: any = {};
    if (include) params.include = JSON.stringify(include);
    return this.http.get(environment.adminUrl + `addons/${id}`, { params });
  }

  create(data: any): Observable<any> { return this.http.post(environment.adminUrl + 'addons', data); }
  update(id: any, data: any): Observable<any> { return this.http.put(environment.adminUrl + `addons/${id}`, data); }
  delete(id: any): Observable<any> { return this.http.delete(environment.adminUrl + `addons/${id}`); }
}
